...
export function AssetInventoryGallery({ kind }: Readonly<{ kind: InventoryKind }>) {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-5">
    ...
    <DetailDrawer open={creating} title={config.action} onClose={() => setCreating(false)}><CreateInventoryRecord kind={kind} onSubmit={(asset) => { setAssets((current) => [asset, ...current]); setCreating(false); setToast(asset.status === 'Registered' ? `${asset.id} was saved as a draft record in frontend mock mode. It is not saved to the database.` : `${asset.id} was added to ${config.title} in frontend mock mode. It is not saved to the database.`); }} /></DetailDrawer>
    <Toast message={toast} />
  </div>;
}
