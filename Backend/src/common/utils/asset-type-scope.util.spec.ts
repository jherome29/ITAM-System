...
describe('resolveAssetTypeScope', () => {
  it('scopes IT_PERSONNEL to ICT only', () => {
    expect(resolveAssetTypeScope(UserRole.IT_PERSONNEL)).toEqual([AssetType.ICT]);
  });

  it('scopes PROPERTY_CUSTODIAN to Fixed and Supplies', () => {
    expect(resolveAssetTypeScope(UserRole.PROPERTY_CUSTODIAN)).toEqual([AssetType.FIXED, AssetType.SUPPLIES]);
  });

  it('scopes PROPERTY_OFFICER to Fixed and Supplies', () => {
    expect(resolveAssetTypeScope(UserRole.PROPERTY_OFFICER)).toEqual([AssetType.FIXED, AssetType.SUPPLIES]);
  });
